...
from random import *
#these variables get a certain value and they are all a part of the starter code
NUMCARDS = 52
DECK = 0
PLAYER = 1
COMP = 2
#these variables are all a part of the starter code with them being used through out the code
cardLoc = [0] * NUMCARDS
...
def clearDeck():
#cardLoc variable gets index 0 multiplied by the value of variable NUMCARDS
  cardLoc=[0]*NUMCARDS
#function assignCard gets defined
def assignCard(y):
#keepGoing variable gets value True
  keepGoing = True
#while loop is introduced with: “while keepGoing”, keepGoing value is true so the while loop begins
  while keepGoing:
...
# print the value of number + one of the value from rankName list + one of the value from suitName list + playerName index the cardLoc index of the value of number
      print (number , " " + rankName[k] +" of "+ suitName[i] + " "+ playerName[cardLoc[number]])
...
#for in in the range of 52
  for i in range (52):
    ...
    if cardLoc[i] == y:
#print the rankName index i mod 13, of, and the suitName i divided by 13 using integer division
      print(rankName[i%13]+ " of " + suitName[i//13])
#this is all a part of the stater code calling all of the functions
def main():
  clearDeck()
# this is for assigning a card to the computer and player, it will do for i in range 5 to assign each person five cards
  for i in range(5):
    assignCard(PLAYER)
    assignCard(COMP)
#these functions are called for showing the deck and which ones are for computer and which ones are for the player
    showDeck()
    showHand(PLAYER)
    showHand(COMP)
